...
class Instrument:
    ...
    def note_off(self, midi_note, midi_vel=0):  # pylint: disable=unused-argument
        ...
        if voice:
            ...
            self.voices.pop(
                midi_note
            )  # TODFIXME: need to run filter after release cycle
...
class PolyWaveSynth(Instrument):
    ...
    def load_patch(self, patch):
        ...
        lfo1 = synthio.Math(
            synthio.MathOperation.SCALE_OFFSET, raw_lfo1, 0.5, 0.5
        )  # unipolar
        self.wave_lfo = lfo1
        ...
        for osc1, osc2, filt_env in self.voices.values():
            ...
            # else simple osc wave mixing between two waveforms
                ...
                if self.waveformB:
                    ...
                    if osc2:
                        osc2.waveform[:] = lerp(
                            self.waveformA, self.waveformB, wave_mix
                        )
    ...
    def note_on(self, midi_note, midi_vel=127):
        ...
        amp_env = synthio.Envelope(
            attack_time=self.patch.amp_env.attack_time,
        ...
        filt_env_wave = Waves.from_ar_times(
            self.patch.filt_env.attack_time, self.patch.filt_env.release_time
        )
        filt_env_time = (
            self.patch.filt_env.attack_time + self.patch.filt_env.release_time
        )
        ...
        osc2 = synthio.Note(
            frequency=f * self.patch.detune, waveform=self.waveform, envelope=amp_env
        )
